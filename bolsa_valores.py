...
def get_acoes():
    return jsonify(bolsa.acoes), 200
...
#             pedido = request.get_json()
#             if "Lista" == pedido:
#                 logger.info(CIANO + f'Pedido de lista de ações recebido da {pedido["id"]}' + RESET)
#                 threading.Thread(target=self.enviar_acoes, args=(pedido['id'],)).start()
#             else:
#                 logger.info(CIANO + f'Pedido de compra recebido da {pedido["id"]}' + RESET)
#                 threading.Thread(target=self.comprar_acoes, args=(pedido,)).start()
#         except Exception as e:
#             logger.info(VERMELHO + f'Falha ao processar o pedido: {e}' + RESET)
#             return jsonify({'status': 'erro', 'mensagem': str(e)}), 500
#         return jsonify({'status': 'ok'}), 200

#     def comprar_acoes(self, pedido):
#         acao = pedido['acao']
#         quantidade = pedido['quantidade']
#         if acao in self.acoes:
#             if self.acoes[acao]['quantidade'] >= quantidade:
#                 self.acoes[acao]['quantidade'] -= quantidade
#                 logger.info(VERDE + f'Compra de {quantidade} ações {acao} realizada com sucesso pela {pedido["id"]}' + RESET)
#             else:
#                 logger.info(VERMELHO + f'Ação {acao} não tem quantidade suficiente disponível para a venda' + RESET)
#         else:
#             logger.info(VERMELHO + f'Ação {acao} não existe' + RESET)

# @app.route('/acoes', methods=['POST'])
# def acoes():
#     bv.comprar_acoes(request.get_json())
#     return jsonify({'status': 'ok'}), 200
